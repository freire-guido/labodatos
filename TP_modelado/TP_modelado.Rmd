...
points(x = 1:3, y = predict(mod_tip, newdata = data.frame(property_type = c("Casa", "Departamento", "PH"))), pch = 18)
```
...
datos$pred = (mod_pos$fitted.values - min(mod_pos$fitted.values)) / (max(mod_pos$fitted.values) - min(mod_pos$fitted.values))
plot(lat ~ lon, data = datos, col = rgb(pred, 0, 1 - pred))
# legend("topright", legend = sort(mod_pos$fitted.values), col = col_pos)
```
