...
Me animo a agrupar a Puerto Madero y Mataderos por un lado, y el resto de los barrios en otro, segun la cantidad de ambientes media
¿Se repetira esto cuando miro la superficie total por barrio?
...
```{r}
aggregate(surface_total ~ l3, data = datos_barrios, FUN = function(x) c(promedio = mean(x), mediana = median(x), rango = range(x)))
```
...
plot(surface_total ~ price, data = datos_puerto, log = "xy", col = col_operaciones[datos_puerto$operation_type], main = "Superficie total y precio (Puerto Madero)")
legend("topleft", legend = operaciones, col = col_operaciones, pch = 1)
```

Es clarisimo que el manchon "mas caro" de Puerto Madero se atribuye a las propiedades en venta. Aun no tengo claro a que se deben los otros dos.